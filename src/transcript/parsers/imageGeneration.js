...
function mediaFromImageGenerationSource(candidate, context = {}) {
  ...
  const localMedia = context.mediaFromLocalFilePath?.(candidateText, context.mediaPolicy);
  if (localMedia) return localMedia;

  const resolved = context.resolveMentionedFilePath?.(candidateText, context.cwd);
  const resolvedMedia = resolved ? context.mediaFromLocalFilePath?.(resolved, context.mediaPolicy) : null;
