...
export function createTranscriptNormalizer(context = {}) {
  ...
  function normalizeTranscriptItem(item, cwd = '', mediaPolicy = {}) {
    const type = item.type ?? 'unknown';
    const base = { id: item.id ?? item.call_id ?? item.callId, type };
    const itemContext = { ...normalizerContext, cwd, mediaPolicy: { ...mediaPolicy, cwd: mediaPolicy.cwd || cwd } };
    ...
    if (type === 'agentMessage') {
      return {
        ...base,
        phase: item.phase,
        text: truncate(context.rewriteLocalFileReferences?.(item.text, cwd, itemContext.mediaPolicy) ?? item.text),
      };
    }
  ...
  function normalizeTranscriptTurn(turn, steeredMessages = [], attachments = [], cwd = '', mediaPolicy = {}) {
    return {
      ...
      items: mergeTurnAttachments((turn.items ?? []).map((item) => normalizeTranscriptItem(item, cwd, mediaPolicy)), attachments),
    };
  }
